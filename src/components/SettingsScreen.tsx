 ...
 * plainly the same furniture — they are two panes of one screen now, so
 * looking alike is the least they owe each other. The segmented controls are
 * this file's own —
...
interface SettingsScreenProps {
  /* Whether the *screen* is open, which includes the time spent on the About
     pane beside this one. Both panes stay mounted for the whole visit — that
     is what makes coming back from About instant instead of a rebuild — so
     this is what tells the one piece of work in here (measuring the caches)
     when a visit has started. */
  open: boolean
  /* One level back, which from the root pane means leaving the screen. */
  onDismiss: () => void
  /* About is reached from the last row of this pane rather than from its own
     ...
     its own pane rather than a group down here: a headline, a demo and a
...
export function SettingsScreen({ open, onDismiss, onOpenAbout }: SettingsScreenProps) {
  ...
  return (
    <>
      <ScreenBar id="settings-title" title="Settings" leading="close" onDismiss={onDismiss} />
      <div className="screen__body">
        <div className="screen__content">
          ...
          {/* Last, and outside the groups above, because it is the one row here
              ...
              it used to take opening a second screen to find. */}
